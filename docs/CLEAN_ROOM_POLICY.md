...
| Aether Fast Input | `docs/features/AETHER_FAST_INPUT.md`, user-authored fast input requests including Saiko+ from the user's old Aether client, Lewn+ and Zeni$h+ from user-provided player feedback | DDNet prediction, input and render-position APIs; existing TClient fast input integration points kept as licensed project context; user-owned old Aether Saiko+ behavior used for equivalence | TClient, Adaptive and Saiko+ input hub updated on 2026-06-15; Lewn+ experimental mode added on 2026-06-27; Zeni$h+ experimental mode added for v1.1.4 |
