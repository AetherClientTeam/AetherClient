...
Aether Fast Input is managed from one input hub with TClient, Aether Adaptive, Saiko+, Control, Lewn+ and Zeni$h+ modes. It does not delay real input and does not switch to full server-side rendering.

Adaptive keeps movement and hook/fire horizons fixed while applying visual correction filtering for small and medium prediction corrections. Saiko+ preserves the old user-authored Aether behavior with tick-style amount units such as `1.10`. Lewn+ is an experimental direct-response mode for extreme play: movement and action use the same immediate fast input horizon, while local render correction follows sharply enough to avoid a delayed feel. Zeni$h+ is an experimental soft-flow mode: it keeps movement light with a softer local render path, keeps hook/dummy response close to Lewn+, and adds a short A/D brake boost so stopping can stay crisp without making normal movement hard.
...
- `ae_lewn_plus_others`
- `ae_zenish_plus_amount`
- `ae_zenish_plus_correction`
- `ae_zenish_plus_others`
...
- Mode-specific tuning is isolated: Adaptive, Saiko+, Control, Lewn+ and Zeni$h+ do not read each other's amount, correction, smoothing or assist settings.
...
- Zeni$h+ uses `ae_zenish_plus_amount / 100.0` ticks for the local movement offset and a small Lewn-like action/dummy floor for hook and drag response. Its default `1.00` amount keeps the base movement light, while the default `30%` correction keeps local render catch-up soft. Releasing A/D or switching directly between A and D gives a short internal brake boost so stops feel more immediate without changing Lewn+, Saiko+ or Control.
