...
void CMod::OnFire(bool Pressed)
{
	...
	const CGameClient::CClientData *pSelectedClient = nullptr;
	float SelectedClientScore = -INFINITY;
	if(GameClient()->m_Snap.m_SpecInfo.m_Active || Player.m_Team == TEAM_SPECTATORS)
	{
		...
		for(const CGameClient::CClientData &Other : GameClient()->m_aClients)
		{
			...
			if(Score > SelectedClientScore)
			{
				SelectedClientScore = Score;
				pSelectedClient = &Other;
			}
		}
	}
	else
	...
	m_ModWeaponActiveId = pSelectedClient->ClientId();
